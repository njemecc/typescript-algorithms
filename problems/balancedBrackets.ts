function isBalanced(s: string): string {
  ...
  const arrayOfBrackets = s.split("");

  if (
    !openings.includes(arrayOfBrackets[0]) ||
  ...
  let otvoreneZagrade = [arrayOfBrackets[0]];

  for (let i = 0; i < arrayOfBrackets.length - 1; i++) {
    if (!openings.includes(arrayOfBrackets[i + 1])) {
      //zatvorena zagrada

      //proverim da li je ova zatvorena istog tipa kao i poslenja otvorena

      const indexZatvorene = closings.indexOf(arrayOfBrackets[i + 1]);
